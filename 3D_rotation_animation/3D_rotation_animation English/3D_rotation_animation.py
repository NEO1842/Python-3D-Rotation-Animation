"""
3D rotating shapes animation using Tkinter (extended version).
Interactive animation for selecting and rotating multiple shapes.
"""
...
class 八面体(Shape3D):
    ...
    def __init__(self):
        ...
        self.edges = [
            ...
            [0, 4], [2, 4], [1, 4], [3, 4],  # upper hemisphere
            [0, 5], [2, 5], [1, 5], [3, 5]   # lower hemisphere
        ]
        self.faces = [
...
class 正三角形(Shape3D):
    """3D shape made of three connected triangles"""

    def __init__(self):
        super().__init__()
        # build three triangles from an equilateral base and one apex
        h = math.sqrt(3) / 2
        self.vertices = [
            [0, -0.7, 0],              # apex (top)
            [0, 0.35, 2 * h / 3],      # base front
            [-0.5, 0.35, -h / 3],      # base left
            [0.5, 0.35, -h / 3]        # base right
        ]

        # three triangles: (0,1,2), (0,2,3), (0,3,1)
        self.edges = [
            [0, 1], [0, 2], [0, 3],
            [1, 2], [2, 3], [3, 1]
        ]
...
class 円柱(Shape3D):
    """Cylinder (high-resolution wireframe)"""
...
class 円錐(Shape3D):
    """Cone (high-resolution wireframe)"""
    ...
    def __init__(self):
        ...
        self.vertices = [[0, height/2, 0]]  # apex
        self.edges = []
        self.faces = []
...
class 球(Shape3D):
    """Sphere (high-resolution wireframe)"""

    def __init__(self):
        super().__init__()
        radius = 1.0
        segments = 16  # translated comment

        self.vertices = []
        self.edges = []
        self.faces = []

        # latitude lines (more horizontal segments)
        lat_segments = segments // 2  # latitude segments
        ...
        # latitude lines (connect points at same latitude)
        ...
        # longitude lines (connect points at same longitude)
        for lon in range(segments):
            for lat in range(lat_segments):
                current = lat * points_per_lat + lon
        ...
        # faces (latitude/longitude grid quads)
        for lat in range(lat_segments):
            for lon in range(segments):
...
class トーラス(Shape3D):
    """Torus (high-resolution donut shape)"""

    def __init__(self):
        super().__init__()
        major_radius = 1.5  # major radius
        minor_radius = 0.5  # minor radius
        major_segments = 12  # more major-circle segments
        minor_segments = 8   # more minor-circle segments
...
class 正十二面体(Shape3D):
    ...
    def __init__(self):
        ...
        # use nearest vertex pairs as edges (dodecahedron has 30 edges)
        dist2_table = {}
        min_dist2 = float("inf")
        ...
        # extract pentagon cycles from edge graph as faces
...
class 正二十面体(Shape3D):
    ...
    def __init__(self):
        ...
        # use nearest vertex pairs as edges (icosahedron has 30 edges)
        dist2_table = {}
        min_dist2 = float("inf")
        for i in range(len(self.vertices)):
            for j in range(i + 1, len(self.vertices)):
        ...
        # extract triangular faces from edge graph (icosahedron has 20 faces)
        adjacency = [set() for _ in range(len(self.vertices))]
        for i, j in self.edges:
            adjacency[i].add(j)
            adjacency[j].add(i)
...
class 三角柱(Shape3D):
    ...
    def __init__(self):
        ...
        self.vertices = [
            [0, y_top, 2 * h / 3],      # upper front
            [-0.5, y_top, -h / 3],      # upper left
            [0.5, y_top, -h / 3],       # upper right
            [0, y_bottom, 2 * h / 3],   # lower front
            [-0.5, y_bottom, -h / 3],   # lower left
            [0.5, y_bottom, -h / 3]     # lower right
        ]
...
class RotatingShape3D:
    """3D rotating shape animation"""

    def __init__(self, root):
        self.root = root
        self.root.title("3D Rotating Shapes Animation - Extended")
        ...
        # shape selection
        tk.Label(self.control_frame, text="Shape:", **label_style).grid(row=0, column=0, padx=5)
        self.shape_var = tk.StringVar(value="Cube")
        self.shape_menu = tk.OptionMenu(self.control_frame, self.shape_var,
                                       "Cube", "Triangle", "Tetrahedron", "Octahedron",
                                       "Cylinder", "Cone", "Sphere", "Torus", "Dodecahedron",
                                       "Icosahedron", "Triangular Prism", "Pentagonal Pyramid",
        ...
        # rotation speed controls (default values)
        self.default_speed_x = 0.01
        self.default_speed_y = 0.01
        self.default_speed_z = 0.01

        tk.Label(self.control_frame, text="X Speed:", **label_style).grid(row=1, column=0, padx=5)
        ...
        tk.Label(self.control_frame, text="Y Speed:", **label_style).grid(row=1, column=2, padx=5)
        ...
        tk.Label(self.control_frame, text="Z Speed:", **label_style).grid(row=1, column=4, padx=5)
        ...
        # reset button
        tk.Button(
            self.control_frame, text="Reset", command=self.reset_rotation, **button_style
        ).grid(row=1, column=6, padx=12, pady=2)
        tk.Button(
            self.control_frame, text="Exit", command=self.exit_app, **button_style
        ).grid(row=1, column=7, padx=(0, 8), pady=2)

        # info display
        self.info_label = tk.Label(
            root,
            text="Keyboard: 1-0,Q,W switch shapes, Arrow keys adjust speed, F toggles faces (with shadows), R resets rotation, E exits, F11 fullscreen, ESC leaves fullscreen",
        ...
        self.current_shape = self.shapes["Cube"]
    ...
    def convex_hull_2d(self, points):
        """Compute 2D convex hull with monotone chain"""
    ...
    def _bind_speed_scale(self, scale_widget, speed_var, axis):
        """Configure mouse/focus behavior for speed sliders"""
    ...
    def _on_speed_scale_pointer(self, event, scale_widget, speed_var, axis):
        """Jump to click position and follow during drag"""
    ...
    def _adjust_active_speed_with_arrow(self, keysym):
        """Adjust active slider with arrow keys"""
        axis_to_var = {
            "x": self.speed_x_var,
            "y": self.speed_y_var,
            "z": self.speed_z_var
    ...
    def _rotate_and_project_without_numpy(self, vertices, center_x, center_y, scale, focal_length):
        """Rotation/projection fallback when NumPy is unavailable"""
        rotated_vertices = []
        screen_points = []
    ...
    def _draw_simple_faces_without_numpy(self, rotated_vertices, screen_points):
        """Simple face rendering fallback without NumPy"""
        face_draw_items = []
    ...
    def animate(self):
        ...
        if np is None:
            ...
            self.canvas.create_text(
                350, 30, text=f"Current Shape: {self.shape_var.get()}",
        ...
        # apply NumPy rotation matrices (X -> Y -> Z)
        ...
        # fixed light source (directly above object)
        light_direction = np.array([0.0, -1.0, 0.0], dtype=np.float64)
        light_direction /= np.linalg.norm(light_direction)
        incoming_light = -light_direction
        shadow_plane_y = -2.0

        # draw shadows only when faces are enabled
        object_center = None
        if show_faces:
            object_center = rotated_np.mean(axis=0)

            # for torus keep the hole via face projection; others use silhouette shadows
            if self.shape_var.get() == "Torus":
        ...
        # draw faces (natural diffuse reflection)
        if show_faces:
            face_draw_items = []
        ...
        # show shape name
        self.canvas.create_text(350, 30, text=f"Current Shape: {self.shape_var.get()}",
                               fill='white', font=('Arial', 12))
...
def main():
    """Main function"""
    if np is None:
        print("NumPy was not found. Starting in simplified rendering mode. Install it with `pip install numpy` for full-quality shading.")
    root = tk.Tk()
